...
namespace Helper
{
    ...
    {
        public static async Task<int> GetApiRecordCount(
            string url,
            string filter,
            string bearertoken
        )
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                recordcount =
                    myresponseparsed != null
                    && myresponseparsed is Newtonsoft.Json.Linq.JObject
                    && myresponseparsed.TotalResults != null
                        ? Convert.ToInt32(myresponseparsed.TotalResults)
                        : 0;
            }
        ...
        public static async Task<IDictionary<string, int>> GetRecordCountfromDB(
        ...
        )
        {
            ...
            try
            {
                ...
                var reducedcount = await QueryFactory
                    ...
                    .When(
                        sources.Count > 0 && odhtype != "odhactivitypoi",
                        q => q.SourceFilter_GeneratedColumn(sources)
                    )
                    .When(
                        sources.Count > 0 && odhtype == "odhactivitypoi",
                        q => q.SyncSourceInterfaceFilter_GeneratedColumn(sources)
                    )
                    .When(
                        tags.Count > 0 && odhtype == "odhactivitypoi",
                        q => q.WhereArrayInListOr(tags, "gen_tags")
                    )
                    .CountAsync<int>();

                //Get Closed
                var closedcount = await QueryFactory
                    .Query()
                    .From(table)
                ...
                //Get Open
                var opencount = await QueryFactory
                    .Query()
                    .From(table)
                    .Where("gen_licenseinfo_closeddata", false)
