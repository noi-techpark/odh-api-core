...
namespace Helper
{
    public class GetAdditionalTypeInfo
    {
        ...
        )
        {
            ...
            var subtypedata = await subtypequery.GetFirstOrDefaultAsObject<SmgPoiTypes>();
            ...
            if (subtypedata != null)
            {
                ...
                var maintypedata = await maintypequery.GetFirstOrDefaultAsObject<SmgPoiTypes>();

                var validtags = await ODHTagHelper.GetODHTagsValidforTranslations(
                    QueryFactory,
                    new List<string>(),
