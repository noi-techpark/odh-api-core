...
namespace OdhApiCoreTests.Helper
{
    public class PostgresSQLWhereBuilderTests
    {
        ...
        public void CreateActivityWhereExpression_LoggedUser()
        {
            ...
            Assert.Equal(
                "SELECT * FROM \"activities\" WHERE ((gen_source <> 'lts') OR (gen_source = 'lts' AND gen_reduced = $$))",
                result.RawSql
            );
        }
        ...
        public void CreateActivityWhereExpression_Anonymous()
        {
            ...
            Assert.Equal(
                "SELECT * FROM \"activities\" WHERE ((gen_source <> 'lts' AND (gen_licenseinfo_closeddata IS NULL OR gen_licenseinfo_closeddata = $$)) OR (gen_source = 'lts' AND gen_reduced = true AND ((gen_licenseinfo_closeddata IS NULL OR gen_licenseinfo_closeddata = $$))))",
                result.RawSql
            );
        }
