...
namespace OdhApiImporter.Helpers.DSS
{
    public class DSSImportHelper : ImportHelper, IImportHelper
    {
        ...
        public DSSImportHelper(ISettings settings, QueryFactory queryfactory, string table)
        ...
        )
        {
            ...
            if (dssinput != null && dssinput.Count > 0)
            {
                ...
                // Get all Areas
                var arealist = await areaquery.GetAllAsObject<AreaLinked>();
                ...
                foreach (var item in dssinput[0].items)
                {
                    ...
                    if (parsedobject != null)
                    {
                        ...
                        var currentcategories = validcategories.Where(
                            x =>
                                parsedobject.SmgTags
                                    ?.Select(y => y.ToLower())
                                    .Contains(x.Id.ToLower()) ?? false
                        );

                        foreach (
                            var languagecategory in parsedobject.HasLanguage ?? new List<string>()
                        )
                        {
                            ...
                            if (entitytype.ToLower() == "lift")
                                additionalpoiinfo.Novelty = (string)
                                    item["info-text"][languagecategory];
                            if (entitytype.ToLower() == "slope")
                                additionalpoiinfo.Novelty = (string)
                                    item["info-text-winter"][languagecategory];

                            foreach (
                                var smgtagtotranslate in currentcategories.Where(
                                    x => x.DisplayAsCategory == true
                                )
                            )
                            {
                        ...
                        var pgcrudresult = await InsertDataToDB(
                            parsedobject,
                            new KeyValuePair<string, dynamic>((string)item.pid, item)
                        );
                        ...
                        WriteLog.LogToConsole(
                            ...
                            new ImportLog()
                            {
                                ...
                                sourceinterface = "dss." + entitytype + "base",
                                success = true,
                                error = ""
                            }
                        );
                    ...
                    else
                    {
                        WriteLog.LogToConsole(
                            ...
                            new ImportLog()
                            {
                                sourceid = parsedobject?.Id ?? "-1",
                                sourceinterface = "dss." + entitytype + "base",
                                success = false,
                                error = entitytype + " could not be parsed"
                            }
                        );
                    }
                }
            ...
            var idlistdb = await GetAllDSSDataByInterface(
                new List<string>() { "dss" + entitytype + "base" }
            );
        ...
        public async Task<ODHActivityPoiLinked?> ParseDSSDataToODHActivityPoi(dynamic dssinput)
        {
            ...
            var odhactivitypoiindb =
                await mydssquery.GetFirstOrDefaultAsObject<ODHActivityPoiLinked>();
            var odhactivitypoi = default(ODHActivityPoiLinked);

            if (entitytype.ToLower() == "lift")
            {
                odhactivitypoi = ParseDSSToODHActivityPoi.ParseDSSLiftDataToODHActivityPoi(
                    odhactivitypoiindb,
                    dssinput
                );
            }
            ...
            {
                odhactivitypoi = ParseDSSToODHActivityPoi.ParseDSSSlopeDataToODHActivityPoi(
                    odhactivitypoiindb,
                    dssinput
                );
            }
