...
namespace OdhApiImporter.Helpers.LTSAPI
{
    ...
    {
        public LTSApiVenueCategoriesImportHelper(
            ISettings settings,
        ...
        private async Task<List<JObject>> GetVenueCategoriesFromLTSV2()
        {
            ...
            catch (Exception ex)
            {
                WriteLog.LogToConsole(
                    ...
                    {
                        sourceid = "",
                        sourceinterface = "lts.venues.categories",
                        success = false,
                        error = ex.Message,
                    }
                );
            ...
            if (ltsdata != null)
            {
                ...
                foreach (var ltsdatasingle in ltsdata)
                {
                    eventtagdata.AddRange(
                        ltsdatasingle["data"].ToObject<IList<LTSVenueCategory>>()
                    );
                }
                    ...
                    foreach (var idtodelete in idstodelete)
                    {
                        ...
                        if (deletedisableresult.Item1 > 0)
                            WriteLog.LogToConsole(
                                idtodelete,
                                "dataimport",
                                "single.venues.categories.deactivate",
                                new ImportLog()
                                {
                                    sourceid = idtodelete,
                                    sourceinterface = "lts.venues.categories",
                            ...
                            WriteLog.LogToConsole(
                                idtodelete,
                                "dataimport",
                                "single.venues.categories.delete",
                                new ImportLog()
                                {
                                    sourceid = idtodelete,
                                    sourceinterface = "lts.venues.categories",
