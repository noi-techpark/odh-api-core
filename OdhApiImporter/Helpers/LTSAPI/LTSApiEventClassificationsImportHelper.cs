    ...
    public class LTSApiEventClassificationsImportHelper : ImportHelper, IImportHelper
    {
        ...
        private async Task<List<JObject>> GetEventClassificationsFromLTSV2()
        {
            try
            {
                ...
                var ltsdata = await ltsapi.EventClassificationsRequest(dict, true);

                return ltsdata;
            }
            catch (Exception ex)
            {
                WriteLog.LogToConsole("", "dataimport", "single.events.classifications", new ImportLog() { sourceid = "", sourceinterface = "lts.events.classifications", success = false, error = ex.Message });
                return null;
            }
        }
