...
namespace OdhApiImporter.Helpers.LTSAPI
{
    ...
    {
        public LTSApiEventClassificationsImportHelper(
            ISettings settings,
        ...
        private async Task<List<JObject>> GetEventClassificationsFromLTSV2()
        {
            ...
            catch (Exception ex)
            {
                WriteLog.LogToConsole(
                    "",
                    "dataimport",
                    "list.events.classifications",
                    new ImportLog()
                    {
                        sourceid = "",
                        sourceinterface = "lts.events.classifications",
                        success = false,
                        error = ex.Message,
                    }
                );
            ...
            if (ltsdata != null)
            {
                ...
                foreach (var ltsdatasingle in ltsdata)
                {
                    eventtagdata.AddRange(
                        ltsdatasingle["data"].ToObject<IList<LTSEventClassification>>()
                    );
                }
                ...
                if (idlistlts.Count > 0)
                {
                    ...
                    var idlistdb = await GetAllDataBySourceAndType(
                        new List<string>() { "lts" },
                        new List<string>() { "eventclassification" }
                    );
                    ...
                    foreach (var idtodelete in idstodelete)
                    {
                        ...
                        if (deletedisableresult.Item1 > 0)
                            WriteLog.LogToConsole(
                                idtodelete,
                                "dataimport",
                                "single.events.classifications.deactivate",
                                new ImportLog()
                                {
                                    sourceid = idtodelete,
                                    sourceinterface = "lts.events.classifications",
                            ...
                            WriteLog.LogToConsole(
                                idtodelete,
                                "dataimport",
                                "single.events.classifications.delete",
                                new ImportLog()
                                {
                                    sourceid = idtodelete,
                                    sourceinterface = "lts.events.classifications",
        ...
        private async Task<PGCRUDResult> InsertDataToDB(
            ...
            LTSEventClassification eventclassification
        )
        {
            try
            {
