...
namespace OdhApiImporter.Helpers.LTSAPI
{
    ...
    {
        public LTSApiGastronomyDishCodesImportHelper(ISettings settings, QueryFactory queryfactory, string table, string importerURL) : base(settings, queryfactory, table, importerURL)
        {
        ...
        private async Task<UpdateDetail> SaveDataToPG(List<JObject> ltsdata)
        {
            ...
            if (ltsdata != null)
            {
                ...
                foreach (var data in tagdata)
                {
                    ...
                    objecttosave.Types = new List<string>() { "dishcodes" };
                    ...
                    WriteLog.LogToConsole(id, "dataimport", "single.gastronomies.dishcodes", new ImportLog() { sourceid = id, sourceinterface = "lts.gastronomies.dishcodes", success = true, error = "" });
                }

                if (idlistlts.Count > 0)
                {
                    //Begin SetDataNotinListToInactive
                    var idlistdb = await GetAllDataBySourceAndType(new List<string>() { "lts" }, new List<string>() { "dishcodes" });
                    ...
                    foreach (var idtodelete in idstodelete)
                    {
                        ...
                        if (deletedisableresult.Item1 > 0)
                            WriteLog.LogToConsole(idtodelete, "dataimport", "single.gastronomies.dishcodes.deactivate", new ImportLog() { sourceid = idtodelete, sourceinterface = "lts.gastronomies.dishcodes", success = true, error = "" });
                        else if (deletedisableresult.Item2 > 0)
                            WriteLog.LogToConsole(idtodelete, "dataimport", "single.gastronomies.dishcodes.delete", new ImportLog() { sourceid = idtodelete, sourceinterface = "lts.gastronomies.dishcodes", success = true, error = "" });
        ...
        private async Task<PGCRUDResult> InsertDataToDB(TagLinked objecttosave, LTSGastronomyDishes data)
        {
            try
            {                
                //Set LicenseInfo
                ...
                return await QueryFactory.UpsertData<TagLinked>(objecttosave, "tags", rawdataid, "lts.gastronomies.dishcodes.import", importerURL);
            }
            catch (Exception ex)
            {
