...
namespace OdhApiImporter.Helpers.LTSAPI
{
    ...
    {
        public LTSApiCeremonyCodesFacilitiesImportHelper(
            ISettings settings,
        ...
        private async Task<List<JObject>> GetDataFromLTSV2()
        {
            ...
            catch (Exception ex)
            {
                WriteLog.LogToConsole(
                    "",
                    "dataimport",
                    "list.gastronomies.ceremonycodes",
                    new ImportLog()
                    {
                        sourceid = "",
                        sourceinterface = "lts.gastronomies.ceremonycodes",
                        success = false,
                        error = ex.Message,
                    }
                );
        ...
        private async Task<UpdateDetail> SaveDataToPG(List<JObject> ltsdata)
        {
            ...
            if (ltsdata != null)
            {
                ...
                {
                    tagdata.AddRange(
                        ltsdatasingle["data"].ToObject<IList<LTSGastronomyCermenoyCodes>>()
                    );
                }
                ...
                foreach (var data in tagdata)
                {
                    ...
                    WriteLog.LogToConsole(
                        id,
                        "dataimport",
                        "single.gastronomies.ceremonycodes",
                        new ImportLog()
                        {
                            sourceid = id,
                            sourceinterface = "lts.gastronomies.ceremonycodes",
                    ...
                    foreach (var idtodelete in idstodelete)
                    {
                        ...
                        if (deletedisableresult.Item1 > 0)
                            WriteLog.LogToConsole(
                                idtodelete,
                                "dataimport",
                                "single.gastronomies.ceremonycodes.deactivate",
                                new ImportLog()
                                {
                                    sourceid = idtodelete,
                                    sourceinterface = "lts.gastronomies.ceremonycodes",
                            ...
                            WriteLog.LogToConsole(
                                idtodelete,
                                "dataimport",
                                "single.gastronomies.ceremonycodes.delete",
                                new ImportLog()
                                {
                                    sourceid = idtodelete,
                                    sourceinterface = "lts.gastronomies.ceremonycodes",
        ...
        private async Task<PGCRUDResult> InsertDataToDB(
            ...
            LTSGastronomyCermenoyCodes data
        )
        {
            try
            {
