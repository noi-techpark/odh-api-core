...
{
    public class LTSApiCeremonyCodesFacilitiesImportHelper : ImportHelper, IImportHelper
    {
        public LTSApiCeremonyCodesFacilitiesImportHelper(ISettings settings, QueryFactory queryfactory, string table, string importerURL) : base(settings, queryfactory, table, importerURL)
        {
        ...
        private async Task<List<JObject>> GetDataFromLTSV2()
        {
            try
            {
                ...
                var ltsdata = await ltsapi.GastronomyCeremonyRequest(dict, true);

                return ltsdata;
            }
            catch (Exception ex)
            {
                WriteLog.LogToConsole("", "dataimport", "single.gastronomies.ceremonycodes", new ImportLog() { sourceid = "", sourceinterface = "lts.gastronomies.ceremonycodes", success = false, error = ex.Message });
        ...
        private async Task<UpdateDetail> SaveDataToPG(List<JObject> ltsdata)
        {
            ...
            if (ltsdata != null)
            {
                ...
                List<LTSGastronomyCermenoyCodes> tagdata = new List<LTSGastronomyCermenoyCodes>();

                foreach (var ltsdatasingle in ltsdata)
                {
                    tagdata.AddRange(ltsdatasingle["data"].ToObject<IList<LTSGastronomyCermenoyCodes>>());
                }
                ...
                foreach (var data in tagdata)
                {
                    ...
                    WriteLog.LogToConsole(id, "dataimport", "single.gastronomies.ceremonycodes", new ImportLog() { sourceid = id, sourceinterface = "lts.gastronomies.ceremonycodes", success = true, error = "" });
                }

                if (idlistlts.Count > 0)
                {
                    //Begin SetDataNotinListToInactive
                    var idlistdb = await GetAllDataBySourceAndType(new List<string>() { "lts" }, new List<string>() { "ceremonycodes" });
                    ...
                    foreach (var idtodelete in idstodelete)
                    {
                        ...
                        if (deletedisableresult.Item1 > 0)
                            WriteLog.LogToConsole(idtodelete, "dataimport", "single.gastronomies.ceremonycodes.deactivate", new ImportLog() { sourceid = idtodelete, sourceinterface = "lts.gastronomies.ceremonycodes", success = true, error = "" });
                        else if (deletedisableresult.Item2 > 0)
                            WriteLog.LogToConsole(idtodelete, "dataimport", "single.gastronomies.ceremonycodes.delete", new ImportLog() { sourceid = idtodelete, sourceinterface = "lts.gastronomies.ceremonycodes", success = true, error = "" });
        ...
        private async Task<PGCRUDResult> InsertDataToDB(TagLinked objecttosave, LTSGastronomyCermenoyCodes data)
        {
            try
            {                
                //Set LicenseInfo
                ...
                return await QueryFactory.UpsertData<TagLinked>(objecttosave, "tags", rawdataid, "lts.gastronomies.ceremonycodes.import", importerURL);
            }
            catch (Exception ex)
            {
        ...
        private async Task<int> InsertInRawDataDB(LTSGastronomyCermenoyCodes data)
        {
            return await QueryFactory.InsertInRawtableAndGetIdAsync(
                        new RawDataStore()
                        {
                            ...
                            sourceid = data.rid,
                            sourceurl = "https://go.lts.it/api/v1/gastronomies/ceremonies",
                            type = "gastronomies.ceremonycodes",
                            license = "open",
                            rawformat = "json"
                        });
        }        
    ...
    public class LTSGastronomyCermenoyCodes
    {
