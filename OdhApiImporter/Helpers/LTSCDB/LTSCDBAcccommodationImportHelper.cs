...
namespace OdhApiImporter.Helpers.LTSCDB
{
    ...
    {
        public LTSCDBAcccommodationImportHelper(
            ISettings settings,
        ...
        {
            var importlist = GetAccommodationDataCDB.GetHotelChangedfromCDB(
                lastchanged,
                "1",
            ...
            {
                idlist = (
                    from x in importlist.Root.Element("Head").Elements("Data")
                    where
                        Convert.ToDateTime(x.Attribute("A0CoC").Value)
                        >= DateTime.Now.AddDays(-1).Date
                    select x.Attribute("A0RID").Value
                ).ToList();
            }
            else
            {
