...
namespace OdhApiCore.Swagger
{
    public class ReferencedAttributeSchemaFilter : ISchemaFilter
    {
        public void Apply(OpenApiSchema schema, SchemaFilterContext context)
        {
            ...
            {
                var referencedAttribute = info
                    .GetCustomAttributes(false)
                    .FirstOrDefault(attribute => attribute.GetType() == typeof(SwaggerReferenceAttribute));
                if (referencedAttribute is SwaggerReferenceAttribute referenced)
                {
                    ...
                    schema.Extensions.Add("isReferencedTo", new OpenApiString(referenced.Description));                    
                }
            }
        }
    }
