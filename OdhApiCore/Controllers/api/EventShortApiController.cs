...
namespace OdhApiCore.Controllers.api
{
    ...
    public class EventShortController : OdhController
    {
        ...
        )
        {
            ...
            var dataTransformed = resultraw.Select(
                raw =>
                    raw.TransformRawData(
                        language,
                        fields,
        ...
        private JsonRaw OptimizeRoomForApp(JsonRaw eventshortraw)
        {
            ...
            var firstbegindate = eventshort.RoomBooked
                ?.OrderBy(x => x.StartDate)
                .Select(x => x.StartDate)
                .FirstOrDefault();
            var lastenddate = eventshort.RoomBooked
        ...
        [Authorize(
            Roles = "DataWriter,DataCreate,EventShortManager,EventShortCreate,VirtualVillageManager"
        )]
        ...
        public async Task<IActionResult> Post([FromBody] EventShortLinked eventshort)
        {
            try
            {
                if (eventshort != null)
                {
                    ...
                    if (
                        CheckIfUserIsOnlyInRole(
                            "VirtualVillageManager",
                            new List<string>()
                            {
                                "DataWriter",
                                "DataCreate",
                        ...
                        && eventshort.EventLocation != "VV"
                    )
                        throw new Exception(
                            "VirtualVillageManager can only insert Virtual Village Events"
                        );
        ...
        //[ApiExplorerSettings(IgnoreApi = true)]
        [Authorize(
            Roles = "DataWriter,DataCreate,EventShortManager,EventShortModify,VirtualVillageManager"
        )]
        [ProducesResponseType(typeof(PGCRUDResult), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
        ...
        //[ApiExplorerSettings(IgnoreApi = true)]
        [Authorize(
            Roles = "DataWriter,DataCreate,EventShortManager,EventShortDelete,VirtualVillageManager"
        )]
        [ProducesResponseType(typeof(PGCRUDResult), StatusCodes.Status200OK)]
        [ProducesResponseType(StatusCodes.Status400BadRequest)]
        [ProducesResponseType(StatusCodes.Status500InternalServerError)]
                    ...
                    if (myevent != null)
                    {
                        if (myevent.Source != "EBMS")
                        {
                            if (
                                ...
                                && myevent.EventLocation != "VV"
                            )
                                throw new Exception(
                                    "VirtualVillageManager can only delete Virtual Village Events"
                                );
                        ...
                        {
                            if (
                                User.IsInRole("VirtualVillageManager")
                                && myevent.EventLocation == "VV"
                            )
                            {
                                //TODO CHECK IF THIS WORKS
        ...
        private const string bdpserviceurl =
            @"https://mobility.api.opendatahub.bz.it/v2/flat/NOI-Place?select=smetadata&where=sactive.eq.true&limit=-1";
        ...
        private async Task<IDictionary<string, string>> GetBDPNoiRoomsWithLink(string? language)
        {
            try
            {
                ...
                foreach (
                    var room in bdprooms.data.Where(
                        x => !String.IsNullOrEmpty(x.smetadata.todaynoibzit)
                    )
                )
                {
                    var maplink = "";
