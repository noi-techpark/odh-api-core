    ...
    public class RawdataApiController : OdhController
    {
        ...
        public async Task<IActionResult> GetRawdataAsync(
            ...
            [SwaggerEnum(
                new[]
                {
                    "event_euracnoi",
                    "odhactivitypoi-lift",
                    "odhactivitypoi-slope",
                    "odhactivitypoi-museum",
                    "weather",
                    "event_centrotrevi-drin",
                    "ejob",
                }
            )]
                string? type = null,
        ...
        )
        {
            return DoAsyncReturn(async () =>
            {
                ...
                if (latest)
                {
                    var query2 = QueryFactory.Query().From("rawdata").WhereIn("id", query);

                    // Get paginated data
                    var data = await query2.PaginateAsync<RawDataStoreWithId>(
                        page: (int)pagenumber,
                        perPage: pagesize ?? 25
                    );
                ...
                {
                    // Get paginated data
                    var data = await query.PaginateAsync<RawDataStoreWithId>(
                        page: (int)pagenumber,
                        perPage: pagesize ?? 25
                    );
