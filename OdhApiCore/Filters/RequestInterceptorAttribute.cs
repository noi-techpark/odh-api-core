    ...
    public class RequestInterceptorAttribute : ActionFilterAttribute
    {
        ...
        )
        {
            if (
                ...
                && settings.RequestInterceptorConfig
                    .Where(x => x.Action == actionid && x.Controller == controller)
                    .Count() > 0
            )
            {
                foreach (
                    var validconfig in settings.RequestInterceptorConfig.Where(
                        x => x.Action == actionid && x.Controller == controller
                    )
                )
                {
            ...
            foreach (var item in querystrings)
            {
                if (!toexclude.Contains(item.Key))
                {
                    if (item.Key == "pagesize")
                    {
                        ...
                        item.Key == "highlight" || item.Key == "active" || item.Key == "odhactive"
                    )
                    {
